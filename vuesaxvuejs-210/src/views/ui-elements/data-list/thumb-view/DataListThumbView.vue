    ...
    <vs-table ref="table" multiple v-model="selected" pagination :max-items="itemsPerPage" search :data="users">
      ...
      <div slot="header" class="flex flex-wrap-reverse items-center flex-grow justify-between">
        ...
        <div class="flex flex-wrap-reverse items-center">
          ...
          <!-- ADD NEW -->
          <div class="p-3 mb-4 mr-4 rounded-lg cursor-pointer flex items-center justify-between
           text-lg font-medium text-base text-primary border border-solid border-primary" @click="addNewDataSidebar = true">
      ...
      <template slot="thead">
        ...
        <vs-th sort-key="quality">Quality</vs-th>
        <vs-th sort-key="unit">Order Status</vs-th> 
        ...
        <tbody>
          <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
            ...
            <vs-td class="img-container">
              <img :src="getconcat(tr)" class="product-img" />
            </vs-td>
            ...
            <vs-td>
             ...
             <vs-td>
              <p class="product-name font-medium">{{ tr.desc }}</p>
            </vs-td>
            ...
            <vs-td>
              <vs-progress :percent="Number(tr.quality) * 20" :color="getPopularityColor(Number(tr.quality))" class="shadow-md" />
            </vs-td>

            <vs-td>
              <vs-chip :color="getOrderStatusColor(unit)" 
              class="product-order-status">{{ tr.unit }}</vs-chip>
            </vs-td>

            <vs-td>
              <p class="product-price">{{ tr.price }} DH </p>
            </vs-td>
          </vs-tr>
        </tbody>
      </template>
...
export default {
  ...
  methods: {
     getconcat(status) {
      const l = "https://recyclers.herokuapp.com/products/"+ status.user_id + "/"+ status.images;
      return l
    },
  ...
  created() {
    const thisIns = this;
    this.$http.get('http://recyclers.herokuapp.com/api/admin/products')
